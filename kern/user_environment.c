...
DECLARE_START_OF(tst_buddy_system_allocation_1);
DECLARE_START_OF(tst_buddy_system_deallocation_1);
DECLARE_START_OF(tst_buddy_system_deallocation_2);
DECLARE_START_OF(tst_malloc_4);
DECLARE_START_OF(tst_malloc_5);
DECLARE_START_OF(tst_malloc_6);
...
struct UserProgramInfo userPrograms[] = {
		...
		//USER DYNAMIC ALLOCATION USING SMALL & LARGE SIZES
		{ "tm4", "tests malloc (4): start address & allocated frames", PTR_START_OF(tst_malloc_4)},
		{ "tm5", "tests malloc (5): writing & reading values in allocated spaces", PTR_START_OF(tst_malloc_5)},
		{ "tm6", "tests malloc (6): check memory allocation and WS after accessing", PTR_START_OF(tst_malloc_6)},
		//BUDDY SYSTEM for SMALL SIZES ALLOCATIONS
		{ "tmb1", "", PTR_START_OF(tst_buddy_system_allocation_1)},
		{ "tfb1", "", PTR_START_OF(tst_buddy_system_deallocation_1)},
		{ "tfb2", "", PTR_START_OF(tst_buddy_system_deallocation_2)},
...
struct WorkingSetElement*temp_work;

struct Frame_Info* frame_info = NULL;


LIST_FOREACH(temp_work,&e->PageWorkingSetList){

	LIST_REMOVE(&e->PageWorkingSetList,temp_work);
...
struct WorkingSetElement*temp_active;
...
struct WorkingSetElement*temp_second;
...
for (uint32 i = 0 ; i <USER_TOP; i+=(PAGE_SIZE*1024))
	...
	get_page_table(e->env_page_directory,(void *)i,&ptr_table);


	if(ptr_table!=NULL){

	    uint32 *pp;
		frame_info = get_frame_info(e->env_page_directory,(void*)ptr_table,&pp);
	...
	// [4] Free the page DIRECTORY from the main memory
			...
			uint32*pp=NULL;
			frame_info = get_frame_info(e->env_page_directory,(void*)e->env_page_directory,&pp);
			frame_info->references--;
			if(frame_info->references==0){
			free_frame(frame_info);
